...
def _render_tab_export(project, mapped_folders: list[str]) -> None:
    ...
    _render_youtube_publish_block(project, folder_selection)


def _render_youtube_publish_block(
    project,
    folder_selection: tuple[str, ...],
) -> None:
    ...
    with st.expander("⚙️ Modell für YouTube Publish", expanded=False):
        role_settings = render_llm_model_selectbox(
            ...
            key=f"yt_publish_model_{project.id}",
        )
        if st.button("Modell speichern", key=f"yt_publish_model_save_{project.id}"):
    ...
    generate_clicked = st.button(
        ...
        key=f"yt_publish_generate_{project.id}",
        type="primary",
        use_container_width=True,
                ...
                if not merged.ready:
                    ...
                    if not any(
                        ...
                        st.warning(
                            ...
                            "Bitte zuerst den finalen Voice-over-Plan bestätigen."
                        )
                    else:
                        log_heavy_operation(
    ...
    st.text_input(
        ...
        key=f"yt_title_display_{project.id}_{document.llm_run_id}",
        disabled=True,
    )
    st.text_area(
        ...
        height=280,
        key=f"yt_desc_display_{project.id}_{document.llm_run_id}",
    )
    st.text_area(
        f"Hashtags ({len(document.hashtags)}/{YOUTUBE_HASHTAGS_MAX_CHARS})",
        value=document.hashtags,
        height=80,
        key=f"yt_hash_display_{project.id}_{document.llm_run_id}",
    )
