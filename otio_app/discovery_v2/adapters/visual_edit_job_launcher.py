"""Discovery V2 launcher for Phase 12 visual edit jobs."""
...
WorkerKind = Literal["visual_edit_plan", "humanity_review", "feasibility_check"]


class VisualEditJobLauncher:
    """One active visual-edit thread per project."""
    ...
    def _worker_fn(self, worker: WorkerKind) -> Callable[[Path, str], object]:
        from otio_app.discovery_v2.jobs.visual_edit_worker import process_visual_edit_run

        return lambda project_root, run_id: process_visual_edit_run(
            project_root,
            run_id,
            worker=worker,
        )
...
_LAUNCHER: VisualEditJobLauncher | None = None
_LAUNCHER_LOCK = threading.Lock()


def get_visual_edit_job_launcher() -> VisualEditJobLauncher:
    global _LAUNCHER
    with _LAUNCHER_LOCK:
        if _LAUNCHER is None:
            _LAUNCHER = VisualEditJobLauncher()
        return _LAUNCHER


def reset_visual_edit_job_launcher_for_tests() -> None:
    global _LAUNCHER
    with _LAUNCHER_LOCK:
        _LAUNCHER = None
...
__all__ = [
    "VisualEditJobLauncher",
    "get_visual_edit_job_launcher",
