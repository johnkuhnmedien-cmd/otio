"""Worker: Video-Transcode-Intake (video-h264-v1) für Discovery V2."""
...
def _reuse(
    ...
    reused = asset.model_copy(
        update={
            ...
            "error_code": "reused",
            "error_message": "Kanonische video-h264-v1-Ausgabe wiederverwendet.",
            "processed_at": _now(),
        }
    )
...
def _validate_existing_output(
    *,
    ...
    try:
        ...
        audio = evaluate_video_audio_policy(source_probe)
        validate_video_h264_output_policy(
            source=source_probe,
            output=out_probe,
            expected_audio=audio,
        )
    except (MediaProbeAdapterError, VideoTranscodeError):
...
def _process_one_asset(
    ...
    if (asset.media_kind or "").lower() != MediaKind.VIDEO.value:
        return _fail(
            conn,
            asset,
            code="unsupported_media_kind",
            message=f"Medientyp ist kein Video: {asset.media_kind}",
            extras=extras,
        )
    ...
    try:
        evaluate_video_audio_policy(source_probe)
    ...
    if existing is not None and _is_completed_status(existing.status):
        if existing.working_relative_path != rel_working:
            return _fail(
                ...
                message="Registry-Pfad stimmt nicht mit kanonischem Transcode-Pfad überein.",
                extras=extras,
            )
        ...
        return _fail(
            ...
            asset,
            code="working_media_conflict",
            message=f"Vorhandene Transcode-Ausgabe widerspricht Policy ({reason}).",
            extras=extras,
        )
    ...
    if working_abs.exists():
        try:
            ...
            ok_repair, reason_repair = _validate_existing_output(
        ...
        if not ok_repair:
            return _fail(
                ...
                code="working_media_conflict",
                message=(
                    "Widersprüchliche Final-Datei am kanonischen Transcode-Pfad "
                    f"({reason_repair})."
                ),
                extras=extras,
            )
    ...
    try:
        ...
        return _fail(
            ...
            message=f"Transcode-Datei veröffentlicht, Registry fehlgeschlagen: {exc}",
            extras=extras,
            audio_policy=result.audio_policy,
            timecode_policy=result.timecode_policy,
        )
...
def process_video_transcode_run(project_root: Path, run_id: str) -> IntakeRunRecord:
    ...
    try:
        ...
        if run.scope != INTAKE_RUN_SCOPE_VIDEO_TRANSCODE_ONLY:
            raise ValueError(
                f"Intake-Run-Scope ist nicht video_transcode_only: {run.scope}"
            )

        if not ffmpeg_available() or not ffmpeg_encoder_available("libx264"):
            code = (
                "ffmpeg_not_found"
                if not ffmpeg_available()
                else "ffmpeg_encoder_unavailable"
            )
            ...
            for asset in assets:
                if asset.status in {
                    ...
                    _fail(
                        ...
                        asset,
                        code=code,
                        message="FFmpeg/libx264 nicht verfügbar.",
                        extras=extras,
                    )
            run = run.model_copy(
                update={
        ...
        for asset in assets:
            if (
                ...
                or (asset.media_kind or "").lower() != MediaKind.VIDEO.value
            ):
                skipped_asset = asset.model_copy(
                    update={
                        "status": IntakeRunAssetStatus.SKIPPED,
                        "error_code": "skipped_non_video_transcode",
                        "error_message": "Nur Video-Transcode-Items werden verarbeitet.",
                        "processed_at": _now(),
                    }
                )
        ...
        run = run.model_copy(
            update={
                ...
                "error_summary": (
                    None
                    if failed == 0
                    else f"{failed} Video-Transcode-Asset(s) fehlgeschlagen."
                ),
            }
        )
        update_intake_run(conn, run)
