...
def build_timeline_items_for_folder(
    ...
    *,
    ...
    """Baut alle Timeline-Items einer Sektion (Narration + aufgeteiltes Outro)."""
    errors: list[str] = []
    section_id = section_id_for_folder(folder_name)
    items: list[TimelineItem] = []
    cursor = 0.0
    ...
    voice_end = narration[-1].voice_end_sec if narration else 0.0
    outro_items, outro_errors = build_outro_timeline_items(
        folder_name=folder_name,
        voice_file=voice_file,
        voice_end_sec=voice_end,
        section_cursor_sec=cursor,
...
def assign_global_timeline_positions(
    ...
    *,
    ...
    """Setzt globale timeline_in/out für zusammengeführte Sektionen."""
    cursor = section_start_sec
...
def shots_from_timeline_items(items: list[TimelineItem]) -> list[EditPlanShot]:
    ...
    for item in items:
        ...
        elif item.type in {"video_shot", "image_shot"}:
            shots.append(
                EditPlanShot(
                    voice_file=item.voice_file,
                    folder=item.folder_name,
