...
Download, ohne Gemini. Spiegel der Classic-Logik in
``cut_plan_generic_fallback_service``, angepasst an Enhanced (Accepted +
Funnel-Report statt CutPlanDocument).
"""
...
class AssetUsageLedger:
    ...
    def note_use(self, asset_id: str, path: str) -> None:
...
def build_asset_usage_ledger(project: Project) -> AssetUsageLedger:
    """Zählt bereits verplante / Accepted Assets für max_asset_usage."""
    ...
    for supplement in list_export_ready_supplements(project):
        ...
        aid = str(supplement.provider_asset_id or supplement.candidate_id or "").strip()
        if path:
...
def apply_generic_fallback_to_gap(
    ...
    *,
    ...
    active_ledger.note_use(inventory_id, str(media_path))
    return GenericGapFallbackResult(
        gap_id=gap_id,
        status="filled",
        candidate=candidate,
