"""Sequenzielle Enhanced-Pipeline: Brief → … → Funnel → Karten → Timing → Musik → OTIO → YouTube.
...
def _recover_chapter_timing_after_failure(
    ...
    *,
    ...
    """
    cut_provider, cut_model = llm_cut_provider_model(
        project, folder_name=chapter_name
    )
    checkpoint("timing")
    emit(
        "timing",
        f"Python Timing für „{chapter_name}“ fehlgeschlagen — neuer LLM-Cut "
        f"({cut_provider}:{cut_model})…",
