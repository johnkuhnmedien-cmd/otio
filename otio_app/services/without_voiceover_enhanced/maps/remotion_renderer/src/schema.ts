...
export const MapTransitionSchema = z.object({
  ...
  styleVersion: z.literal("otio-vintage-map-v16"),
  viewBounds: z.tuple([
  ...
  geographyLabels: z
    .array(
      z.object({
        ...
        longitude: z.number().min(-180).max(180),
        latitude: z.number().min(-85).max(85),
        numericId: z.string().max(3).optional().default(""),
        atlasName: z.string().max(80).optional().default(""),
      }),
    )
    .max(12)
    .default([]),
...
export const defaultMapTransition: MapTransitionProps = {
  ...
  styleVersion: "otio-vintage-map-v16",
  viewBounds: [
    [-125, 24],
    [-66, 50],
  ],
