...
def test_schema_12_to_13_preserves_data_and_is_idempotent(tmp_path: Path, temp_db_path: Path) -> None:
    root = tmp_path / "Project"
    (root / "Media").mkdir(parents=True)
    ...
    try:
        assert reg_db.read_schema_version(conn3) == "13"
    finally:
        conn3.close()


def test_schema_13_tables_exist_without_dramaturgy_or_visual_beats(tmp_path: Path, temp_db_path: Path) -> None:
    root = tmp_path / "Project"
    (root / "Media").mkdir(parents=True)
    ...
    try:
        ...
        assert {"analysis_consent_events", "model_analysis_attempts", "visual_observations"}.issubset(tables)
