...
def test_resolve_llm_cut_prefix_four_includes_intro_and_three_chapters(
    tmp_path: Path, cut_plan_defaults_dir: Path
) -> None:
    root = tmp_path / "Greece"
    work = root / DEFAULT_ENHANCED_WORK_SUBDIR
    work.mkdir(parents=True)
    folders = ["Athens", "Thessaloniki", "Crete", "Rhodes"]
    for folder in folders:
        (root / folder).mkdir()
    project = Project(
        id="pt-greece-cut-prefix-four",
        name="PT_Greece",
        project_root=str(root),
        work_dir=str(work),
        project_mode=ProjectMode.WITHOUT_VOICEOVER_ENHANCED,
    ...
    assert resolve_llm_cut_model_id(project, is_intro=True) == "openai:gpt-5.6-sol"
    assert resolve_llm_cut_model_id(project, folder_name="Athens") == "openai:gpt-5.6-sol"
    assert (
        resolve_llm_cut_model_id(project, folder_name="Thessaloniki")
        == "openai:gpt-5.6-sol"
    )
    assert resolve_llm_cut_model_id(project, folder_name="Crete") == "openai:gpt-5.6-sol"
    assert resolve_llm_cut_model_id(project, folder_name="Rhodes") == "openai:gpt-5.6-terra"
