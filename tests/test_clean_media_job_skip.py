...
from otio_app.services.clean_media import CLEAN_STATUS_CLEAN, CLEAN_STATUS_OK, save_clean_media_manifest
...
def test_process_job_does_not_skip_replaced_original(tmp_path: Path) -> None:
    project = _project(tmp_path)
    folder = project.project_root_path / "Grand Canyon"
    (folder / "clip.mp4").unlink()
    old = folder / "Grand_Canyon_Asset12.mp4"
    new = folder / "Grand_Canyon_Asset00012.mov"
    new.write_bytes(b"licensed")
    old_clean = project.work_dir_path / "clean" / "Grand_Canyon" / "Grand_Canyon_Asset12.mp4"
