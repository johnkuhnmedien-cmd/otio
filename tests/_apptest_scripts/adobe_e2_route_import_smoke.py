"""E2 AppTest-Smoke: echte Import-Seite + JobManager.start (kein Result-Seed).

Netzwerkgrenze Adobe wird gemockt. Der Start-Button ruft denselben
`mgr.start(...)` auf wie in Produktion; der Worker führt
`download_research_import()` aus.
"""
...
ROOT = Path(os.environ.get("ADOBE_E2_SMOKE_ROOT", "/tmp/adobe-e2-route-smoke"))
PROJECT_ID = "e2-route-smoke"
TINY = (
    Path(__file__).resolve().parents[1] / "fixtures" / "adobe_tiny_valid.mp4"
)
    ...
    def fake_urlopen(request, timeout=20):
        ...
        if host == "stock.adobe.io" and "/Download/" in url:
            raise _HTTPError(
                302,
                headers={
                    "Location": "https://cdn.example-adobe-test.test/e2.mp4?sig=REDACT_ME"
                },
                url=url,
            )
