...
def test_build_edit_plan_retries_gemini_on_timing_validation_failure(
    ...
    def fake_validate_folder_plan_timing(*args, **kwargs):
        ...
        return TimelineValidationResult(status=ValidationStatus.OK)
    ...
    with (
        ...
        patch(
            "otio_app.services.edit_plan_builder.validate_folder_plan_timing",
            side_effect=fake_validate_folder_plan_timing,
        ),
        patch(
    ...
    assert any("Korrektur nach Timing-Validierung" in note for note in document.plan_generation_notes)
